...
const Phone = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .post(api_route + "/phone/" + sessionStorage.getItem("id"), {
          phoneNumber,
          phoneNetwork,
        })
        .then(() =>
  ...
  return (
    <div className="w-full  lg:w-1/2 flex flex-col items-center justify-center  rounded-md ">
      {!verfiy ? (
        ...
        >
          <h2 className="font-bold text-xxl my-2  ">
            {" "}
            تأكيد رقم هاتف مقدم الطلب
          </h2>
          <p className="py-1 text-xs font-bold text-gray-500">
            <span> تم تسجيل بيانات الوثيقة بنجاح !</span>
            <span>لمتابعة الطلب يرجي إدخال رقم مقدم الطلب بشكل صحيح</span>
          </p>
          ...
          <div className="flex justify-start p-1 py-3 items-start gap-y-2 w-full flex-col">
            <span className="font-bold">مشغل شبكة الجوال *</span>
            <select
              value={phoneNetwork}
              required
